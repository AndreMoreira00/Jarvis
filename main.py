  ...
  while cap.isOpened():
      ...
      if results.multi_hand_landmarks and results.multi_handedness:
          ...
          for hand_landmarks, hand_handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
              ...
              if hand_label == "Right" and hands_system.Map_Ok(h, w, hand_landmarks, frame):
                control_functions.Capture_Photo(frame)
                time.sleep(0.5)
              
              if hand_label == "Left" and hands_system.Map_Positive(h, w, hand_landmarks, frame):
                control_functions.Capture_Video(cap)
                time.sleep(0.5)
                
              if hand_label == "Right" and hands_system.Map_Speak(h, w, hand_landmarks, frame):
