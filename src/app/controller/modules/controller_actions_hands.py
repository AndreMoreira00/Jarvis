...
class ControllerHands:
  ...
  def checkGesture(self, executor: object, frame: np.ndarray, cap: object, h:float, w:float, hand_landmarks:object, hand_label:str)-> None:  # type: ignore
    _checks: list[tuple[object, object, str, int]] = [
      ...
      (lambda: executor.submit(self.basicFunctions.captureVideo, cap, executor), lambda: self.rulesHands._mapPositive(h, w, hand_landmarks), "Left", 30),  # type: ignore
    ...
    for func_exe, func_act, side, cooldown in _checks: 
